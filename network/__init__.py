...
def get_model(network, num_classes, criterion):
    ...
    """
    module = network[:network.rfind('.')]       #jacob/ network.deepv3
    model = network[network.rfind('.') + 1:]    #DeepV3PlusR50
    mod = importlib.import_module(module)       #이걸 통해서 network/deepv3.py를 Import해옴
